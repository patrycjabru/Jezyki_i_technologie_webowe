...
//    $file3=$_POST['file3'];
    ...
    try {
        ...
        foreach ($dirIt as $dir) {
            if($dir=='.' or $dir=='..')
                continue;
            $infoFile=fopen("users/".$dir."/info.txt","r") or die("Unable to open a file!");
...
HTML;
        ...
        $infoFile=file("users/".$blogName."/info.txt") or die("Unable to open a file!");
...
//		echo $date[0].$date[1].$date[2].$date[3].$date[5].$date[6].$date[8].$date[9].$time[0].$time[1].$time[3].$time[4];
	    $allFiles=scandir('users'.'/'.$blogName);
//	    echo $allFiles[2];
...
//	    $fullPath=$path."\users\\".$blogName."\\".$fileName;
	    touch($fullPath);
        $post = fopen("users/".$blogName."/".$fileName, "w") or die("Unable to open file!");
//        echo $content;
        fwrite($post,$content);
